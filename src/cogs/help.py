...
class HelpDropdown(discord.ui.Select):
    def __init__(
        self, help_command: "HelperCommands", options: list[discord.SelectOption]
    ):
        super().__init__(
...
class HelperCommands(commands.MinimalHelpCommand):
        ...
        for cog, command_set in self.get_bot_mapping().items():
            ...
            options.append(
                discord.SelectOption(
                    ...
                    emoji=emoji,
                    description=(
                        cog.description[:100] if cog and cog.description else None
                    ),
                )
            )
    ...
    async def send_command_help(self, command: commands.Command):
        ...
        embed = await self._help_embed(
            title=(
                f"{emoji} {command.qualified_name}" if emoji else command.qualified_name
            ),
    ...
    async def cog_help_embed(self, cog: Optional[commands.Cog]) -> Embed:
        if cog is None:
            return await self._help_embed(
                title="No category", command_set=self.get_bot_mapping()[None]
            )
        emoji = getattr(cog, "COG_EMOJI", None)
        return await self._help_embed(
